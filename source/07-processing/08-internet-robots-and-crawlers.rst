...
The bots repository will be updated no more frequently than once per quarter, and no less frequently than once per year. Any new additions SHOULD be excluded as soon as possible, and MUST be excluded from the subsequent fix, feature or breaking release of the Code of Practice, as described under the continuous maintenance process in :numref:`maintenance`. Usage data prior to the report provider implementing the updated bots repository does not need to be reprocessed to exclude the new additions unless doing so will remove significant bot usage.
